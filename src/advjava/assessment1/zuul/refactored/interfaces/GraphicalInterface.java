...
public class GraphicalInterface extends Application implements UserInterface {
    ...
    private static final int IMAGE_NODE_HEIGHT = 50;
    private static final int IMAGE_NODE_WIDTH = 50;
    ...
    public boolean update() {
        Out.out.logln("Updating interface.");
        return false;
    }
    ...
    private Node getDisplayItem(Resource resource) {
        ...
        gridbox.setPadding(new Insets(NODE_TOP_OFFSET, NODE_RIGHT_OFFSET, NODE_BOTTOM_OFFSET, NODE_LEFT_OFFSET));
        gridbox.setHgap(10);
        gridbox.setVgap(10);

        Text text = new Text(resource.getName() + (resource.getDescription() != null ? System.lineSeparator() + resource.getDescription() : ""));

        text.setFont(fontManager.getFont("Bauhaus 93"));
        text.setStyle("-fx-fill: bef4c8;");

        ImageView iv = new ImageView(resource.getImage());
        iv.setFitWidth(IMAGE_NODE_WIDTH);
        iv.setFitHeight(IMAGE_NODE_HEIGHT);

        gridbox.add(iv, 0, 0);

        if (resource instanceof Item) {
            Item item = (Item) resource;
            text.setText(String.format("%s%sWeight: %d", text.getText(), System.lineSeparator(), item.getWeight()));
            ...
            gridbox.add(button, 0, 1);
        }

        gridbox.add(text, 1, 0);
//        r.setArcWidth(20);
//        r.setArcHeight(20);

        sp.getChildren().addAll(gridbox);
        return sp;
