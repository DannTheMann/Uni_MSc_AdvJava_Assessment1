...
/**
 * The XMLManager is designed to load and interact 
 * with all other management across the game to add
 * rooms/items/characters into the game.
 * 
 * It's robust so that certain features of characters/items/rooms
 * can be left out but neccesary features will be noted if missing 
 * and errors will be thrown in accordance to this.
 * @author Daniel
 *
 */
public abstract class XMLManager {
	...
	/**
	 * Load all rooms found in the XML file
	 * and add them to the room manager
	 * @param rm The room manager
	 * @throws MalformedXMLException If XML is malformed or is missing key nodes
	 */
						...
						if(innerElement.getElementsByTagName("exit") != null){
							...
							for(int j = 0; j < innerElement.getElementsByTagName("exit").getLength(); j++){
								...
								if(rName.equals(name)){
									// If the exit points to the current room then ignore it
									continue;
								}
								
								// Need to check whether this room exists in order
								// to use it as a reference for an exit
								if (rm.hasRoom(rName)){
									// it does exit, then load it
									room = rm.getRoom(rName);
								}else{
									// create new room otherwise and fill it in later
	...
	/**
	 * Load all characters found in the XML file
	 * and add them to the character manager
	 * @param rm The character manager
	 * @throws MalformedXMLException If XML is malformed or is missing key nodes
	 */
		...
		try {
			...
			for (int i = 0; i < nList.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (eElement.getElementsByTagName("inventory").item(0) != null) {
						...
						for(int j = 0; j < innerElement.getElementsByTagName("item").getLength(); j++){
							...
							if((Main.game.getItemManager().hasItem(iName))){
								...
								// else throw exception, can't add an item that doesn't exist
					...
					// If the weight for this character was specified, set their max weight
					if(weight != null){
	...
	/**
	 * Shorthand method to handle getting of elements from an
	 * Element object, fills in a lot of duplicated code
	 * otherwise and handles any null cases.
	 * @param eElement The element to check inside
	 * @param str The text of the element we're looking for
	 ...
	 * Shorthand method to handle getting of elements from an
	 * Element object, fills in a lot of duplicated code
	 * otherwise and handles any null cases. If multiple
	 * indexs are present in the node, use this method to
	 * specify which index.
	 * @param eElement The element to check inside
	 * @param str The text of the element we're looking for
	 ...
	 * @throws Exception If something goes wrong I/O
	 */
